...
import {
  FiUser, FiMapPin, FiGlobe, FiLink, FiLayers, FiInfo, FiActivity, FiArrowLeft, FiShield
} from 'react-icons/fi';
  ...
  if (!profileData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-[#050505] gap-4">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">Node Not Found</p>
        <button onClick={() => router.back()} className="text-[10px] font-black text-orange-500 uppercase underline">Return</button>
      </div>
    );
  }
        ...
        <div className="mb-8 flex flex-col gap-6">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 dark:border-white/5 pb-8">
                <div>
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.3em] mt-1">Verified Public Profile Intelligence</p>
                </div>
                <div className="px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-md">
                    <span className="text-[9px] font-black text-green-500 uppercase tracking-widest flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></div> Status: {user.status}
                    </span>
                </div>
            </div>
        </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="bg-white dark:bg-[#0c0c0c] border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden rounded-lg">
              ...
              <div className="h-32 w-full bg-gray-100 dark:bg-white/5 relative border-b border-gray-100 dark:border-white/5">
                <img
                  src={getImageUrl(user.profile?.coverImage) || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000'}
              ...
              <div className="px-6 pb-8 -mt-12 flex flex-col items-center text-center relative z-10">
                ...
                <div className="mt-8 w-full grid grid-cols-2 gap-3">
                  <div className="p-3 bg-gray-50 dark:bg-white/5 rounded-md border border-gray-100 dark:border-white/10 text-left">
                    <p className="text-[8px] font-bold text-gray-400 uppercase tracking-widest mb-1">Access Role</p>
                    <span className="text-[10px] font-black dark:text-white uppercase">{user.role}</span>
                  </div>
                  <div className="p-3 bg-gray-50 dark:bg-white/5 rounded-md border border-gray-100 dark:border-white/10 text-left">
            ...
            <div className="p-6 bg-white dark:bg-[#0c0c0c] border border-gray-100 dark:border-white/5 rounded-lg shadow-sm">
              ...
              <div className="space-y-5">
                <div className="flex justify-between items-center group">
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest group-hover:text-orange-500 transition-colors">Total Inventory</span>
                  <span className="text-xs font-black dark:text-white uppercase tracking-widest">{profileData.listingsCount || 0} Nodes</span>
                </div>
                <div className="h-[1px] w-full bg-gray-100 dark:bg-white/5"></div>
                <div className="flex justify-between items-center group">
                  <span className="text-[10px] font-bold text-gray-500 uppercase tracking-widest group-hover:text-orange-500 transition-colors">Activation Date</span>
            ...
            <div className="bg-white dark:bg-[#0c0c0c] border border-gray-100 dark:border-white/5 shadow-sm rounded-lg overflow-hidden">
              <div className="p-8 md:p-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 animate-in fade-in duration-700">
                  ...
                  <InfoRow label="Primary Protocol" value={user.profile?.language} icon={Languages} />
                  <InfoRow label="Inventory Volume" value={`${profileData.listingsCount || 0} ACTIVE UNITS`} icon={FiLayers} />
                  
                  <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                     <InfoRow label="Global Link" value={user.profile?.websiteLink} icon={Globe} isLink />
                     <InfoRow label="Network Node" value={user.profile?.socialLink} icon={FiLink} isLink />
                  </div>
                  ...
                  <div className="md:col-span-2 mt-4 p-6 bg-gray-50 dark:bg-black/20 border border-gray-100 dark:border-white/5 rounded-md">
                    ...
                    <p className="text-[11px] font-bold leading-relaxed dark:text-gray-400 italic">
                      {user.profile?.bio || 'System intelligence has not provided a biography for this node yet...'}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-12 flex items-center gap-4">
                    ...
                    <p className="text-[8px] font-black text-gray-400 uppercase tracking-[0.3em]">End of Dossier</p>
                </div>
              </div>
            </div>
          </div>
