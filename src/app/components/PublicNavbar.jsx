    ...
    <nav className="fixed top-0 left-0 w-full bg-white dark:bg-[#0a0a0a] border-b border-[#F2F2F2] dark:border-[#1F1F1F] z-50">
      <div className="flex items-center justify-between px-6 py-4 relative">
        ...
        <div className="flex items-center space-x-2">
          <h1 className="text-xl font-semibold text-[#1F1F1F] dark:text-white">
            WCM
          </h1>
          ...
          {showSearch ? (
            ...
            >
              <input
                ...
                className="w-full border-b-2 border-[#F57C00] bg-transparent
                text-[#1F1F1F] dark:text-white placeholder-gray-400
                py-2 pr-20 focus:outline-none"
              />
              <button
                type="submit"
                className="absolute right-2 top-1/2 -translate-y-1/2
                text-sm font-medium text-[#F57C00]"
              >
                Search
              </button>
            </form>
            ...
            >
              {menuItems.map((item) => (
                <Link
                  ...
                  className="text-sm font-medium text-[#1F1F1F] dark:text-white
                  hover:text-[#F57C00] transition"
                >
                  {item}
                </Link>
              ))}
        ...
        <div className="flex items-center space-x-3">
          {showSearch ? (
            <FiX
              className="cursor-pointer h-9 w-9 p-2 rounded-full
              border border-[#F57C00] text-[#F57C00]"
              onClick={() => setShowSearch(false)}
            />
          ) : (
            <FiSearch
              className="cursor-pointer h-9 w-9 p-2 rounded-full
              hover:bg-[#F2F2F2] dark:hover:bg-[#1F1F1F]"
          ...
          {isLoggedIn ? (
            <Link
              ...
              bg-[#F57C00] text-white text-sm font-medium hover:opacity-90"
            >
              Become a Creator
            </Link>
          ) : (
            <Link
              href="/login"
              className="hidden md:block px-4 py-2 rounded-lg
              border border-[#F57C00] text-[#F57C00] text-sm font-medium
              hover:bg-[#F57C00] hover:text-white transition"
            >
              Login
            </Link>
          )}
          ...
          <FiMenu
            className="cursor-pointer h-9 w-9 p-2 rounded-full
            hover:bg-[#F2F2F2] dark:hover:bg-[#1F1F1F] md:hidden"
      ...
      {/* Mobile Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-2/3 bg-white dark:bg-[#0a0a0a]
      ...
      >
        ...
        <div className="flex flex-col space-y-4 px-6">
          ...
          {isLoggedIn ? (
            ...
            <Link
              href="/login"
              className="mt-4 px-4 py-2 rounded-lg border border-[#F57C00]
              text-[#F57C00] text-center"
            >
              Login
            </Link>
          )}
