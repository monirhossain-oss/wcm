...
export default function AdminRegions() {
    ...
    useEffect(() => {
        ...
        const fetchRegions = async () => {
            ...
            setCurrentPage(1);
            try {
                const res = await api.get(`/api/admin/regions/by-category/${selectedCategory._id}`);
                setRegions(res.data);
            } catch (err) {
                console.error(err);
            } finally {
    ...
    const currentItems = regions.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
    ...
    const handleAddRegion = async (e) => {
        ...
        setSubmitting(true);
        try {
            const res = await api.post('/api/admin/regions', {
            ...
            alert(err.response?.data?.message || 'Error adding region');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const handleUpdate = async (id) => {
        ...
        setUpdatingId(id);
        try {
            const res = await api.put(`/api/admin/regions/${id}`, { title: editData.title });
            setRegions((prev) => prev.map((r) => (r._id === id ? res.data : r)));
            setEditingId(null);
        } catch (err) {
            alert('Update failed');
        } finally {
        ...
        <div className="max-w-5xl mx-auto pb-20 font-sans">
            ...
            <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-[#0c0c0c] p-2 rounded-lg border border-gray-100 dark:border-white/10 shadow-xl overflow-hidden">
                    <form onSubmit={handleAddRegion} className="flex flex-col sm:flex-row gap-2">
                        <div className="relative flex-1">
                            <FiMapPin className="absolute left-6 top-1/2 -translate-y-1/2 text-orange-500" size={18} />
                            <input
                                type="text"
                                value={formData.title}
                                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                                placeholder={`Type region name for ${selectedCategory?.title}...`}
                ...
                <div className="bg-white dark:bg-[#0c0c0c] rounded-lg border border-gray-100 dark:border-white/10 overflow-hidden shadow-lg">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-white/5 border-b border-gray-100 dark:border-white/10 text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                                <th className="px-8 py-5">Region Label</th>
                        ...
                        <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                            {loadingRegions ? (
                                ...
                                currentItems.map((region) => (
                                    <tr key={region._id} className="group hover:bg-gray-50/50 dark:hover:bg-orange-500/5 transition-colors">
                                        <td className="px-8 py-6">
                                            {editingId === region._id ? (
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <span className="font-black uppercase text-[12px] tracking-tight dark:text-white italic">{region.title}</span>
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex justify-end gap-3">
                                                {editingId === region._id ? (
                                                    <>
                                                        <button onClick={() => handleUpdate(region._id)} className="w-10 h-10 flex items-center justify-center text-green-500 bg-green-500/10 rounded-md hover:bg-green-500 hover:text-white transition-all">
                                                            {updatingId === region._id ? <FiLoader className="animate-spin" /> : <FiCheck size={18} />}
                                                        </button>
                                                    ...
                                                    <>
                                                        <button onClick={() => { setEditingId(region._id); setEditData({ title: region.title }); }} className="w-10 h-10 flex items-center justify-center text-gray-400 bg-gray-100 dark:bg-white/5 rounded-md hover:text-blue-500 hover:bg-blue-500/10 transition-all">
                                                            <FiEdit2 size={16} />
                                                        </button>
                                                        <button onClick={() => handleDelete(region._id)} className="w-10 h-10 flex items-center justify-center text-gray-400 bg-gray-100 dark:bg-white/5 rounded-md hover:text-red-500 hover:bg-red-500/10 transition-all">
                                                            <FiTrash2 size={16} />
                                                        </button>
                                                    </>
                                                )}
                    ...
                    {totalPages > 1 && (
                        <div className="p-6 border-t border-gray-100 dark:border-white/10 flex items-center justify-between bg-gray-50/30 dark:bg-transparent">
                            <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Showing {currentItems.length} of {regions.length}</p>
