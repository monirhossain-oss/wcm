...
import { 
  ...
  FiArrowUpRight, FiPieChart, FiTrendingUp, FiLayers, FiShield
} from 'react-icons/fi';
import {
...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-20 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-gray-100 dark:border-white/5 pb-8">
        <div>
          ...
          <p className="text-[10px] font-black text-gray-400 tracking-[0.3em] uppercase mt-2 italic">
            System Status: <span className="text-green-500">Operational</span> // Data Refreshed
          </p>
        </div>
      ...
      {/* 1. Metric Intelligence Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {metricCards.map((card, i) => (
          <div key={i} className={`${card.isInverted ? 'bg-orange-600 text-white' : 'bg-white dark:bg-[#0c0c0c] border border-gray-100 dark:border-white/5'} p-6 rounded-lg shadow-sm relative overflow-hidden group hover:border-orange-500/30 transition-all`}>
            <div className="relative z-10">
              <p className={`text-[10px] font-black uppercase tracking-[0.2em] ${card.isInverted ? 'text-white/60' : 'text-gray-400'}`}>{card.label}</p>
              <h3 className={`text-3xl font-black mt-2 italic tracking-tighter ${card.isPrimary && !card.isInverted ? 'text-orange-500' : ''}`}>{card.value}</h3>
              <p className={`text-[9px] mt-3 font-bold uppercase tracking-widest italic ${card.isInverted ? 'text-white/80' : 'text-gray-500'}`}>{card.trend}</p>
            </div>
            <card.icon className={`absolute -right-6 -bottom-6 ${card.isInverted ? 'text-white/10' : 'text-gray-100 dark:text-white/2'} group-hover:scale-110 transition-all duration-700`} size={120} />
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white dark:bg-[#0c0c0c] rounded-lg border border-gray-100 dark:border-white/5 p-8 shadow-sm">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
            <div>
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">Financial Growth Pulse</h3>
              <p className="text-2xl font-black dark:text-white mt-1 italic uppercase tracking-tighter">Revenue <span className="text-orange-500">&</span> User Matrix</p>
            </div>
          ...
          <div className="h-[380px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={charts.revenueAndUsers} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <defs>
                ...
                <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fontSize: 9, fontWeight: 800, fill: '#888' }} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fontWeight: 800, fill: '#888' }} />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0c0c0c', border: '1px solid #ffffff10', borderRadius: '4px', fontSize: '10px' }}
                />
                <Area type="monotone" dataKey="revenue" stroke="#f97316" strokeWidth={4} fill="url(#colorRev)" />
                <Area type="monotone" dataKey="users" stroke="#64748b" strokeWidth={2} strokeDasharray="5 5" fill="transparent" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 bg-white dark:bg-[#0c0c0c] rounded-lg border border-gray-100 dark:border-white/5 p-8 shadow-sm">
          <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 mb-8 flex items-center gap-2">
            <FiPieChart className="text-orange-500" /> Revenue Dossier
          </h3>
          <div className="h-[280px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip />6                <Legend verticalAlign="bottom" wrapperStyle={{ fontSize: '9px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '1px', paddingTop: '20px' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
          <div className="mt-8 space-y-4">
            <StatusRow label="System Integrity" value="99.9%" color="text-green-500" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={charts.categories} layout="vertical" margin={{ left: -20 }}>
                ...
                <YAxis dataKey="name" type="category" axisLine={false} tickLine={false} tick={{ fontSize: 9, fontWeight: 900, fill: '#888', textTransform: 'uppercase' }} width={120} />
                <Tooltip cursor={{fill: '#88888808'}} contentStyle={{ backgroundColor: '#0c0c0c', border: '1px solid #ffffff10', color: '#fff', fontSize: '10px' }} />
        ...
        <div className="lg:col-span-5 bg-white dark:bg-[#0c0c0c] rounded-lg border border-gray-100 dark:border-white/5 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">High-Priority Assets</h3>
            <Link href="/admin/listings" className="text-[10px] font-black text-orange-500 uppercase tracking-widest hover:underline">Intercept All</Link>
          </div>
          <div className="space-y-4">
            {charts.topPromoted?.map((listing, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-gray-50 dark:bg-white/2 border border-gray-100 dark:border-white/5 rounded-md hover:border-orange-500/30 transition-all group">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-[11px] font-black uppercase truncate max-w-[160px] tracking-tight dark:text-white">{listing.name}</p>
                    <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-sm mt-1 inline-block ${listing.type === 'PPC' ? 'bg-orange-500/10 text-orange-500' : 'bg-purple-500/10 text-purple-400'}`}>
                      {listing.type} Scheme
                    </span>
                  </div>
                </div>
                <div className="text-right">
                   <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Yield Score</p>
                   <p className="text-sm font-black text-orange-500 italic tracking-tighter">{listing.score}</p>
                </div>
              </div>
            ))}
            {!charts.topPromoted?.length && (
              <div className="py-16 text-center border border-dashed border-gray-200 dark:border-white/10 rounded-md">
                <p className="text-[9px] uppercase font-black tracking-widest text-gray-400 italic">No Trending Transmissions</p>
              </div>
            )}
          </div>
        </div>
